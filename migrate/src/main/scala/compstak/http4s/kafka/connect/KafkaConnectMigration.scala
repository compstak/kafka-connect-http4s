...
final class KafkaConnectMigration[F[_]: ContextShift](client: KafkaConnectClient[F], config: KafkaConnectMigration.Configuration)(implicit val F: Sync[F]) {

  def migrate: F[Unit] =
    buildDeprecatedConfigs.map(_ ++ buildActiveConfigs)
      .flatMap { actions =>
        actions.evalMap {
  ...
  private[this] def listActiveConfigs = Stream
    .fromIterator[F](Files.walk(config.path).iterator().asScala)
    .filter(_.endsWith(".json"))

  private[this] def buildActiveConfigs = listActiveConfigs
    .evalMap(loadConfig)
    .evalMap { case (name, conf) =>
      conf.asObject match {
        case Some(obj) =>
  ...
  private[this] def loadConfig(p: Path): F[(String, Json)] = {
    val content = readInputStream(Sync[F].delay(this.getClass.getResourceAsStream(p.toAbsolutePath.toString)), 4096, Blocker.liftExecutionContext(ExecutionContext.global))
      .through(utf8Decode)
      .compile
      .lastOrError
